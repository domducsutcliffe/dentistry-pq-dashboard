...
// Live manual refresh status indicator check using Shields.io (CORS enabled)
function updateRefreshStatus() {
  ...
  fetch(`https://img.shields.io/github/actions/workflow/status/domducsutcliffe/dentistry-pq-dashboard/refresh-data.yml?cb=${Date.now()}`)
    ...
    .catch(err => {
      console.warn("Failed to check manual refresh status badge:", err);
    });
}

// Bind click listener immediately (since app.js runs as a deferred module)
...
// Fallback listener for page focus and periodic 10-second checks
window.addEventListener("focus", updateRefreshStatus);
setInterval(updateRefreshStatus, 10000);
